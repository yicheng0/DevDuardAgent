...
class DevGuardAgentApp {
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '向 DevGuardAgent 描述告警、日志或排障问题';
        }
    }
    ...
    async triggerAIOps() {
        ...
        // 添加分析中的消息（带旋转动画）
        const loadingMessage = this.addLoadingMessage('DevGuardAgent 正在运行 AI Ops 分析...');
        ...
        try {
            ...
            if (loadingMessage) {
                ...
                if (messageContent) {
                    messageContent.textContent = '抱歉，AI Ops 分析时出现错误：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = 'DevGuardAgent 正在分析，请稍候...';
                if (loadingSubtext) loadingSubtext.textContent = '后端正在处理请求';
                // 防止页面滚动
